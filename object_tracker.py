...
def main(_argv):
    ...
    while True:
        ...
        for bbox, score, class_name, feature in zip(converted_boxes, scores[0], names, features):
            if(class_name not in filtered_class):
                continue
        ...
        for track in tracker.tracks:
            ...
            # else:
            #     continue
            if track.track_id not in id:
                id.append(track.track_id)
            if track.track_id not in overall:
                overall.append((track.track_id))
            class_name = track.get_class()
        ...
        cv2.putText(img, "COUNT: {:d}".format(count), (0, 30),
                    cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 2)
        cv2.putText(img, "ID: {:d}".format(len(id)), (0, 80),
                    cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 2)
        cv2.putText(img, "ALL: {:d}".format(len(overall)), (0, 130),
                    cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 2)
